...
export const esquemaDevolucion = z
  .object({
    ...
    montoReembolsado: dineroNoNegativo,
...
export async function registrarDevolucion(
  ...
  const imagen = await decodificarEvidencia(entrada.evidencia);
  const archivo = await guardarImagen(imagen, "devoluciones");
  try {
    ...
    await eliminarImagen(archivo.ruta).catch(() => undefined);
    throw error;
  }
}
