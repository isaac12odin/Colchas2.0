...
npm run build:android -w movil          # AAB de producción
npm run build:ios -w movil              # iOS de producción
```
...
El usuario resultante tiene rol `ADMINISTRADOR` y puede entrar directamente. Ejecutar de nuevo el seed rota su hash usando los valores explícitos; nunca recupera una clave anterior ni inventa una de respaldo.
...
- Una ruta puede operarse sólo desde la web administrativa sin cobrador. Al asignar un cobrador también aparece en el móvil de esa cuenta; el servidor sigue limitando a cada cobrador a sus rutas y clientas incluso si intenta consultar la API directamente.
...
- Desde el pedido, Administración puede registrar un producto completo sin perder la captura. Un pedido pendiente exige proveedor por artículo; después se registra llegada, preparación y entrega con acciones visibles según el rol.
- Capturar abono consulta los pedidos pendientes de la clienta y permite abrir su acción correspondiente, dejando claro que no entran al saldo hasta la entrega.
...
- Las contraseñas admiten desde 6 caracteres. Administración puede cambiar la clave de cualquier cuenta, incluida la propia; el cambio cierra sus sesiones y no fuerza otro cambio al siguiente acceso.
...
- Web y móvil incluyen capacitación por rol. Los recorridos sencillos son guiados; venta a crédito, abono, entrega, devolución y corrección de sincronización son simuladores completos en memoria que permiten equivocarse, explican la regla y muestran consecuencias ficticias en saldo, inventario y corte sin llamar a la API.
...
| `npm run validar:todo`                   | Tipos, todas las pruebas vigentes y compilación                |
...
| `npm run datos:demo:limpiar`             | Retira datos demo y desactiva sus cuentas sin romper historial |
...
- [Catálogo de errores API](docs/ERRORES_API.md)
- [Invariantes normativas del negocio](docs/INVARIANTES_NEGOCIO.md)
- [Operación y capacitación](docs/OPERACION.md)
- [Producción, respaldos y monitoreo](docs/PRODUCCION.md)
- [Liberación y rollback](docs/RELEASE.md)
- [Privacidad y clasificación](docs/PRIVACIDAD_DATOS.md)
